...
//UUID basically for generating random strinmg name of images to store in firebase
...
export default class AddNewContact extends Component {
  ...
  saveContact = async () => {
    ...
    if (
      ...
      //set uploading process state to true here
      this.setState({ isUploading: true });
      const dbReference = firebase.database().ref();
      const storageRef = firebase.storage().ref();

      //verify that we have image or not then only we call uploadIMage()
      ...
      //push this to database and go back to previous screen i.e HomeScreen
  ...
  //async for not to kepp whole thread busy
  pickImage = async () => {
  ...
  uploadImageAsync = async (uri, storageRef) => {
    //file uploaded with extension we will split that
    const parts = uri.split(".");
    const fileExtension = parts[parts.length - 1];

    //create blob
    //here we are writing old school .then().catch no big deal
    const blob = await new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      //once onloaded we need to resolve it
      xhr.onload = function() {
    ...
    //close blob
    blob.close();
    //take back url and store it to somewhere of fireabsse imaages
