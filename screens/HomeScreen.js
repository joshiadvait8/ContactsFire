...
export default class HomeScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      data: [],
      isLoading: true,
      isListEmpty: false //we are assuming list is not empty initialyy
    };
  }
  ...
  getAllContact = () => {
    ...
    //grab all values and sort them and store the,m n state variable
    let contactRef = firebase.database().ref();
    //if new value get added listener is called "on"
    contactRef.on("value", dataSnapshot => {
      if (dataSnapshot.val()) {
        //whenever datasnapshot come from firebase it holds key value pair wee need to separate that
        let contactResult = Object.values(dataSnapshot.val());
        let contactKey = Object.keys(dataSnapshot.val());
        contactKey.forEach((value, key) => {
          //we are extracting datasnapshot loop through it and we are setting each conact
          //contact[1]["key"] this key contact[2] this key
